...
自托管部署使用基于镜像的 Compose 模板。模板默认使用外部兼容 S3 的存储和环境变量中配置的模型供应商。
...
| 运行时限制      | `UPLOAD_*`, `PARSER_*`, `*_CONCURRENCY`, `DELETION_CLEANUP_*`, `COMPILE_MAX_CONTEXT_CHARS`, `RETRIEVE_*`, `SOURCE_EVIDENCE_*`, `RUNTIME_*`, `PROVIDER_FAILURE_DEGRADED_THRESHOLD` |
