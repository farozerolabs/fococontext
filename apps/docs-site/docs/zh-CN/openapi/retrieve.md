...
| `warnings`                                | 请求级非致命 warning code，例如 `retrieve.index.semantic_not_ready`、`retrieve.rerank_failed` 或 `retrieve.rerank_timed_out`。空 env 禁用 rerank 会通过 trace metadata 呈现。                                 |
