...
| `warnings`                                | Request-level non-fatal warning codes such as `retrieve.index.semantic_not_ready`, `retrieve.rerank_failed`, or `retrieve.rerank_timed_out`. Empty-env rerank disablement is trace metadata, not a warning.     |
