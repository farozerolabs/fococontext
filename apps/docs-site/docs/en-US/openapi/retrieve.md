...
| `trace.stages[].output.store`             | Retrieval backing path such as PostgreSQL lexical search, pgvector semantic search, indexed graph traversal, Redis cache, or bounded source-evidence lookup.                                                                                                                |
| `trace.stages[].output.query_path`        | High-level execution path for the stage, for example optimized bounded retrieval, indexed graph expansion, or cache-backed materialized insight lookup.                                                                                                                     |
...
Production Retrieve and Retrieve Expand do not silently fall back to full in-memory scans. If bounded retrieval, graph indexes, Redis cache state, or source evidence backends are unavailable, clients receive typed warnings or typed errors such as `bounded_retrieval_unavailable`, `graph_index_unavailable`, `durable_backend_unavailable`, or `retrieve_index_not_ready`.
